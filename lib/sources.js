...
const {getOrigin, getOriginUrl, getProviderName, getServiceName, getSwaggerPath} = require('./utils')
...
export function getCatalogsLeads (providers = catalogProviders) {
  const black = load(readFileSync(resolve(__dirname, '../sources/blacklist.yaml')))
...
export function getLeadsAsync (specs) {
  ...
  return getCatalogsLeads(usedCatalogs)
    .then(catalogLeads => {
