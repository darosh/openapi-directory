...
export function runValidate (ctx) {
  assert(!_.isEmpty(ctx.swagger.paths), 'Definition should have operations')
...
function fixSpec (swagger, errors) {
  ...
  _.each(errors, function (error) {
    ...
    try {
      parentValue = jp.get(swagger, _.dropRight(path))
    ...
    switch (error.code) {
      ...
      case 'OBJECT_MISSING_REQUIRED_PROPERTY_DEFINITION':
        if (_.isUndefined(value.properties)) { break }

        newValue = _.clone(value)
        newValue.required = []
        _.each(value.required, function (name) {
          if (!_.isUndefined(value.properties[name])) { newValue.required.push(name) }
        })
        if (_.isEmpty(newValue.required)) { delete newValue.required }
        break
      case 'ONE_OF_MISSING':
      ...
      case 'INVALID_TYPE':
        if (_.nth(path, -1) === 'required' && _.nth(path, -3) === 'properties' &&
          ...
          const objectSchema = jp.get(swagger, _.dropRight(path, 3))

          if (value) {
            objectSchema.required = objectSchema.required || []
            const propertyName = _.nth(path, -2)
...
export function validatePreferred (specs) {
  ...
  Object.keys(preferred).forEach(function (id) {
    ...
    if (Object.keys(versions).length === 1) {
      ...
      assert(_.isUndefined(versions[0]) || versions[0] === true, 'Preferred not true in "' + id + '"')
      return
    }
